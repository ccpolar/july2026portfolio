 ...
 * Skips for reduced motion, and removes itself after 10s regardless, so a
 ...
 * string.
 */
export const INTRO_BOOT_SCRIPT = `(function(){try{var d=document.documentElement,k=${JSON.stringify(INTRO_SEEN_KEY)};if(sessionStorage.getItem(k)||matchMedia('(prefers-reduced-motion: reduce)').matches)return;sessionStorage.setItem(k,'1');d.setAttribute('data-intro','run');setTimeout(function(){d.removeAttribute('data-intro')},10000);document.addEventListener('click',function(e){var t=e.target;if(t&&t.closest&&t.closest('[data-intro-skip]'))d.removeAttribute('data-intro')},true)}catch(e){}})();`
